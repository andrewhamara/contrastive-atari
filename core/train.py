...
from core.supcon import SupConLoss, discretize_values, prepare_supcon_views, filter_small_bins

_supcon_criterion = SupConLoss(temperature=0.07, base_temperature=0.07)
...
def compute_supcon_loss(model, hidden_state, value_logits, config):
    """Compute SupCon loss on hidden state representations, binned by predicted value.

    Args:
        model: EfficientZeroNet with supcon_project() method.
        hidden_state: [batch, channels, h, w] hidden state from dynamics/representation.
        value_logits: [batch, support_size] categorical value output.
        config: config with supcon_coeff, supcon_num_bins, supcon_min_bin_size.

    Returns:
        [batch] per-sample SupCon loss, or zeros if not enough valid bins.
    """
    batch_size = hidden_state.size(0)
    device = hidden_state.device

    # Get scalar value predictions for binning
    with torch.no_grad():
        scalar_values = config.inverse_value_transform(value_logits).squeeze(-1)

    # Discretize into bins
    num_bins = getattr(config, 'supcon_num_bins', 32)
    min_bin_size = getattr(config, 'supcon_min_bin_size', 2)
    value_bins = discretize_values(scalar_values, num_bins=num_bins)

    # Filter out small bins
    valid_mask = filter_small_bins(value_bins, min_bin_size=min_bin_size)

    if valid_mask.sum() < 2 * min_bin_size:
        return torch.zeros(batch_size, device=device)

    # Project valid hidden states
    valid_hidden = hidden_state[valid_mask]
    valid_bins = value_bins[valid_mask]

    embeddings = model.supcon_project(valid_hidden)
    supcon_features = prepare_supcon_views(embeddings, noise_std=0.01)

    loss_val = _supcon_criterion(supcon_features, labels=valid_bins)

    # Broadcast scalar loss back to batch
    per_sample = torch.zeros(batch_size, device=device)
    per_sample[valid_mask] = loss_val
    return per_sample
...
def update_weights(model, batch, optimizer, replay_buffer, config, scaler, vis_result=False, step_count=0):
    ...
    supcon_loss = torch.zeros(batch_size, device=config.device)

    # SupCon loss — only on encoder (representation network) output, not dynamics unrolls.
    # Cosine ramp-up: starts near zero, reaches full strength at warmup_fraction of training.
    supcon_coeff = getattr(config, 'supcon_coeff', 0.0)
    if supcon_coeff > 0:
        warmup_frac = getattr(config, 'supcon_warmup_fraction', 0.5)
        total_steps = config.training_steps + config.last_steps
        if warmup_frac > 0 and step_count < warmup_frac * total_steps:
            # cosine ramp from 0 to 1 over warmup period
            progress = step_count / (warmup_frac * total_steps)
            ramp = 0.5 * (1 - math.cos(math.pi * progress))
            supcon_coeff = supcon_coeff * ramp
        supcon_loss = compute_supcon_loss(model, hidden_state, value, config)
    ...
    loss_data = (total_loss.item(), weighted_loss.item(), loss.mean().item(), 0, policy_loss.mean().item(),
                 ...
                 supcon_loss.mean().item(), supcon_coeff)
    if vis_result:
