...
class BaseConfig(object):
    ...
    def __init__(self,
                 ...
                 supcon_coeff: float = 0.0,
                 supcon_num_bins: int = 32,
                 supcon_min_bin_size: int = 2,
                 supcon_warmup_fraction: float = 0.5,
                 proj_hid: int = 256,
                 proj_out: int = 256,
                 pred_hid: int = 64,
                 pred_out: int = 256,
        ...
        self.supcon_coeff = supcon_coeff
        self.supcon_num_bins = supcon_num_bins
        self.supcon_min_bin_size = supcon_min_bin_size
        self.supcon_warmup_fraction = supcon_warmup_fraction
        self.device = 'cuda'
    ...
    def set_config(self, args):
        ...
        if getattr(args, 'supcon_num_bins', None) is not None:
            self.supcon_num_bins = args.supcon_num_bins
        if getattr(args, 'supcon_min_bin_size', None) is not None:
            self.supcon_min_bin_size = args.supcon_min_bin_size
        if getattr(args, 'supcon_warmup_fraction', None) is not None:
            self.supcon_warmup_fraction = args.supcon_warmup_fraction
