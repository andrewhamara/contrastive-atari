...
def _log(config, step_count, log_data, model, replay_buffer, lr, shared_storage, summary_writer, vis_result):
    ...
    *core_losses, supcon_loss, supcon_coeff_eff = loss_data
    total_loss, weighted_loss, loss, reg_loss, policy_loss, value_prefix_loss, value_loss, consistency_loss = core_losses
    if vis_result:
    ...
    _msg = '#{:<10} Total Loss: {:<8.3f} [weighted Loss:{:<8.3f} Policy Loss: {:<8.3f} Value Loss: {:<8.3f} ' \
           'Reward Sum Loss: {:<8.3f} Consistency Loss: {:<8.3f} SupCon Loss: {:<8.3f} ] ' \
    ...
    _msg = _msg.format(step_count, total_loss, weighted_loss, policy_loss, value_loss, value_prefix_loss, consistency_loss,
                       supcon_loss, replay_episodes_collected, replay_buffer_size, total_num / 1000, config.batch_size, lr)
    ...
    if summary_writer is not None:
        ...
        summary_writer.add_scalar('{}/supcon_loss'.format(tag), supcon_loss, step_count)
        summary_writer.add_scalar('{}/supcon_coeff_effective'.format(tag), supcon_coeff_eff, step_count)
