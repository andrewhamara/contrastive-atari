...
class AtariConfig(BaseConfig):
    def __init__(self):
        super(AtariConfig, self).__init__(
            ...
            policy_loss_coeff=1,
            consistency_coeff=2,
            # supcon (value-bin supervised contrastive)
            supcon_coeff=0.0,
            supcon_num_bins=32,
            supcon_min_bin_size=2,
            supcon_warmup_fraction=0.5,
            # reward sum
    ...
    def new_game(self, seed=None, save_video=False, save_path=None, video_callable=None, uid=None, test=False, final_test=False):
        ...
        if seed is not None:
            try:
                ...
                pass  # gymnasium handles seeding via reset(seed=seed)

        if save_video:
            from gymnasium.wrappers import RecordVideo
            env = RecordVideo(env, video_folder=save_path, name_prefix=str(uid or "video"))
