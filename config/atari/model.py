...
class EfficientZeroNet(BaseNet):
    ...
    def supcon_project(self, hidden_state):
        """Project hidden state for SupCon loss (separate from SPR projection)."""
        if not hasattr(self, 'supcon_projection'):
            # Lazy init — same dims as consistency projection
            self.supcon_projection = nn.Sequential(
                nn.Linear(self.porjection_in_dim, self.proj_hid),
                nn.BatchNorm1d(self.proj_hid),
                nn.ReLU(),
                nn.Linear(self.proj_hid, 128),
            ).to(hidden_state.device)
        flat = hidden_state.view(-1, self.porjection_in_dim)
        proj = self.supcon_projection(flat)
